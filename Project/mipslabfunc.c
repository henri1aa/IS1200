...
/* 
The following code is written by T Björkdahl & H Gidehag 
*/

/* Display the wanted figure, we call this function in labwork in pongwork */
void paint_sprite(uint8_t * sprite, int x, int y, int widthX, int heightY)
{
  int i, j;
  int Ybefore = y;
  int Xbefore = x;
  for(i = 0; i < heightY; i++, x = Xbefore)
  {
    y = Ybefore + i;
   	for (j = 0; j < widthX; j++, x++)
		{
			if(y < 0  || y > 32 || x < 0 || x >= 128)
      {
				continue;
			}
			displaypixels[y][x] |= *((sprite+i*widthX) + j);
    }
  }
}

/* Converts score string into sprite pixels */
void paint_score(int x, int y, int widthX)
{
  ...
  for (j = 0; j < widthX; j++, x++)
  {
    ...
    for(k = 0; k < 8; k++, x++)
    {
      displaypixels[y][x] |= (font[text*8 + k]);
    }
  }
}

/* Converts text into sprite pixels */
...
/* Function to convert score to a string */
/* Code modified from Labworks */
...
/* Sets all pixels to off thus clearing display */
void clear_display()
{
	int i,j;
	for (i = 0; i < 32; i++)
